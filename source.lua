...
function Library.new(title, logoImage)
    local self = setmetatable({
        _config    = Cfg:load(game.GameId),
        _title     = title or "UI",
        _logoImage = logoImage or "rbxassetid://102985234114068",
        _tab       = 0,
        _open      = true,
        _scale     = 1,
        _device    = nil,
...
function Library:_build()
    ...
    LogoIcon.Image=self._logoImage; LogoIcon.BackgroundTransparency=1
    ...
    function self:create_tab(title, icon)
        ...
        function TabManager:create_module(s)
            ...
            KbFrame.InputBegan:Connect(function(inp)
                ...
                local conn; conn=UserInputService.InputBegan:Connect(function(kb)
                    if kb.UserInputType~=Enum.UserInputType.Keyboard then return end
                    conn:Disconnect(); stopDots(); _pick=false; Library._choosing_keybind=false
            ...
            function MM:create_dropdown(s2)
                ...
                function DM:toggle()
                    ...
                    if self._open then
                        ...
                        TweenService:Create(DDArr,TweenService:Create(DDArr,TweenInfo.new(0.5,Enum.EasingStyle.Quint,Enum.EasingDirection.Out),{Rotation=180}):Play() and nil or nil)
                    else
                        MM._mult-=self._listH
                        TweenService:Create(Mod,TweenInfo.new(0.5,Enum.EasingStyle.Quint,Enum.EasingDirection.Out),{Size=UDim2.fromOffset(MOD_W,93+MM._size+MM._mult)}):Play()
                        ...
                        TweenService:Create(DDArr,TweenService:Create(DDArr,TweenInfo.new(0.5,Enum.EasingStyle.Quint,Enum.EasingDirection.Out),{Rotation=0}):Play() and nil or nil)
                    end
                end
                DM._listH=3
            ...
            function MM:create_multi_dropdown(s2)
                ...
                for idx,val in s2.options do
                    ...
                    MinCB.Size=UDim2.fromOffset(11,11); MinCB.AnchorPoint=Vector2.new(0,0.5)
...
-- ============================================================
--  EXAMPLE  --  edit everything below this line freely
-- ============================================================

-- Library.new( TITLE , LOGO IMAGE ID )
-- TITLE        : text shown in the top-left header
-- LOGO IMAGE ID: rbxassetid of the icon next to the title


-- Always call load() last.  It makes the window visible.
main:load()
